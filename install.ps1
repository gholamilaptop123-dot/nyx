...
# Download latest source zip if backend doesn't exist locally
if (-not (Test-Path "$installDir\backend")) {
    ...
    Remove-Item -Path "$env:TEMP\nyx_extract" -Recurse -Force -ErrorAction SilentlyContinue
}
...
Write-Host "Setting up backend and frontend dependencies..." -ForegroundColor Cyan
...
cmd.exe /c "npm install --loglevel=error && npm run build"
...
Write-Host "Service Name:   $taskName (Auto-starts on Windows Boot)" -ForegroundColor Yellow
Write-Host "====================================================" -ForegroundColor Green
