...
[🚀 قابلیت‌های جدید v2.4](#-قابلیت‌های-جدید-در-نسخه-v240) •
...
در این آپدیت بزرگ، بر اساس بازخوردها و پیشنهادات کاربران عزیز، امکانات سفارشی‌سازی پیشرفته، پشتیبانی از دامنه اختصاصی/CDN، پروتکل‌های نسل جدید و ابزارهای برندینگ برای ادمین‌ها و فروشندگان اضافه شده است:

### 🌐 ۱. پشتیبانی کامل از دامنه اختصاصی و CDN کلودفلر (`Custom Domain & CDN Mapping`)
* **حذف کامل نیاز به نمایش IP سرور:** از این پس می‌توانید در بخش تنظیمات، دامنه شخصی یا ساب‌دامین متصل به CDN کلودفلر (مثل `vpn.mydomain.com`) را وارد کنید. سیستم به صورت خودکار تمام لینک‌های VLESS، سابسکریپشن‌های Base64، سینگ‌باکس و کلش را با این دامنه جنریت می‌کند.
* **تنظیم دامنه مستقل به ازای هر اینباند:** اگر چند سرور لبه یا دامنه‌های مختلف برای اپراتورهای گوناگون دارید، می‌توانید برای هر اینباند به صورت مجزا دامنه اختصاصی تعریف کنید.

### ⚡ ۲. تنظیمات دقیق و حرفه‌ای پکت فرگمنت در پنل (`Packet Fragment Tuning`)
* **تنظیم دستی پارامترهای پکت:** بدون نیاز به دستکاری دستی کانفیگ‌ها، مقدار طول پکت (`Fragment Length`) و فاصله زمانی ارسال (`Fragment Interval`) مستقیماً از داخل فرم ساخت و ویرایش اینباند قابل تنظیم است.
* **پریست‌های آماده و تست‌شده اپراتورها:**
  * 📱 **همراه اول (MCI):** `100-200, 10-20` (بهترین پایداری روی نت همراه اول)
  * 📡 **ایرانسل (Irancell):** `50-150, 5-15` (عبور موثر از فیلترینگ ایرانسل)
  * ⚡ **ترافیک داخلی / اینترانت:** `10-60, 2-10`

### 🚀 ۳. ترنسپورت نسل جدید `XHTTP (SplitHTTP)`، `gRPC` و پروتکل `Trojan`
* **پروتکل نوین XHTTP:** جدیدترین استاندارد ضدسانسور هسته Xray برای عبور از شدیدترین اختلالات شبکه و دور زدن مسدودی وب‌سوکت‌ها روی کلودفلر و سرورهای واسط.
* **تنوع پروتکل‌ها:** پشتیبانی هماهنگ از `VLESS`, `VMess`, `Trojan` روی ترنسپورت‌های `TCP Reality`, `WebSocket`, `XHTTP`, `gRPC`.

### 🎨 ۴. شخصی‌سازی کامل صفحه ساب مشتری و برندینگ اختصاصی (`Sub Portal Custom Branding`)
* **برندینگ کامل صفحه مشتری (`/subinfo/:uuid`):** امکان تعیین نام برند یا فروشگاه، قرار دادن آدرس لوگوی اختصاصی و تنظیم ظاهر صفحه اشتراک خریداران.
* **دکمه‌های مستقیم پشتیبانی و تمدید اشتراک:** اتصال مستقیم دکمه‌های شیک به آیدی تلگرام پشتیبانی و کانال تلگرام جهت تمدید سریع سرویس توسط خریداران.
...
* **نوار جستجوی سریع در تب کاربران:** فیلتر آنی کاربران بر اساس نام کاربری، شناسه UUID یا وضعیت فعال/منقضی.
* **نوار جستجوی سریع در تب اینباندها:** جستجوی فوری بر اساس نام کانفیگ، پورت، پروتکل، SNI یا دامنه.

### 📱 ۶. طراحی کاملاً لمسی و بهینه‌سازی‌شده برای موبایل (`Mobile-First UX`)
* بازطراحی و تست تمامی کارت‌ها، مودال‌ها، اسلایدرها و منوهای پنل جهت استفاده بسیار روان و سریع روی انواع گوشی‌های هوشمند.
