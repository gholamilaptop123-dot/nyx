...
### ☁️ 3. استقرار رایگان روی سرورهای ابری (Railway / Render / Docker):

اگر سرور مجازی (VPS) ندارید یا نمی‌خواهید هزینه ماهانه سرور پرداخت کنید، می‌توانید Nyx Panel را روی پلتفرم‌های ابری رایگان مانند **Railway.app** اجرا کنید.

#### 💡 مزایای استقرار روی پلتفرم‌های ابری (Railway):
* 💸 **بدون نیاز به خرید VPS:** بدون پرداخت هزینه سرور اختصاصی، پنل را بالا بیاورید.
* 🛡️ **ضد فیلتر با دامنه اختصاصی و CDN:** ترافیک کاربران در قالب **VLESS WebSocket با پورت استاندارد 443 و گواهی امنیتی TLS** از روی شبکه ابری عبور می‌کند.
* ⚡ **آپ‌تایم و پایداری بالا:** بهینه‌سازی شده با مالتی‌پلکسر هوشمند وب‌سوکت برای دور زدن اختلالات اینترنت ایران.
...
> [!NOTE]
> برای استفاده از Railway تنها به یک **اکانت رایگان گیت‌هاب (GitHub)** نیاز دارید و هیچ‌گونه کارت اعتباری یا شماره خارجی لازم نیست.

1. **ورود به Railway:** وارد سایت [railway.com](https://railway.com) شوید و با کلیک روی گزینه **Login with GitHub** وارد داشبورد شوید.
2. **ساخت پروژه جدید:** روی دکمه بنفش‌رنگ **`+ New Project`** در بالای صفحه کلیک کنید.
3. **انتخاب گیت‌هاب:** گزینه **`Deploy from GitHub repo`** را انتخاب نمایید.
4. **انتخاب مخزن پنل:** نام مخزن را وارد کنید یا در نوار جستجو آدرس زیر را وارد نمایید:
   ```text
   icynetx/Nyx
   ```
   *(یا مستقیماً آدرس `https://github.com/icynetx/Nyx`)*
5. **شروع دیپلوی:** روی دکمه **`Deploy Now`** کلیک کنید. ریلوِی به صورت خودکار شروع به بیلد Dockerfile و اجرای هسته Xray می‌کند (حدود ۶۰ تا ۹۰ ثانیه زمان می‌برد).
6. **دریافت دامنه رایگان اینترنتی (HTTPS):**
   * پس از سبز شدن وضعیت سرویس (`Active 🟢`)، روی باکس پروژه کلیک کنید.
   * به تب **Settings** در بالای صفحه بروید.
   * به پایین اسکرول کرده تا به بخش **Networking** برسید.
   * روی دکمه **Generate Domain** کلیک کنید. ریلوِی یک دامنه رایگان HTTPS (مثلاً `nyx-production.up.railway.app`) برای شما می‌سازد.
7. **ورود به پنل و اتصال کاربران:**
   * دامنه دریافتی را در مرورگر باز کنید.
   * با مشخصات پیش‌فرض زیر وارد شوید:
     * **نام کاربری:** `admin`
     * **کلمه عبور:** `nyx2026!`
   * یک کاربر جدید بسازید یا وارد تب **کاربران** شوید و لینک سابسکریپشن را کپی کنید.
   * کانفیگ‌های `⚡ Railway-Cloud-WSS` به صورت خودکار آماده اتصال مستقیم کلاینت‌ها (v2rayNG, Streisand, Shadowrocket, etc.) هستند!

---

#### 🐳 اجرای کانتینر با Docker / Docker Compose (سرور شخصی):

اگر سرور لینوکس یا سیستم خودتان را دارید و می‌خواهید با داکر اجرا کنید:
